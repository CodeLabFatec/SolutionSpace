...
export default function KanbanBoard() {
  ...
  const getKanbanByDestinationId = (id: any) => {
    if (id === 1 || id === "1")
      return kanban.find((i: any) => i.column === "NEW");
    if (id === 2 || id === "2")
      return kanban.find((i: any) => i.column === "ON HOLDING");
    if (id === 3 || id === "3")
      return kanban.find((i: any) => i.column === "DONE");
    return kanban.at(0);
  };
  ...
  const handleLimpar = (e: any) => {
    ...
    alert.criarConfirmacao({
      title: "Aviso",
      html: "Deseja excluir os chamados em 'Done'?",
      confirmAction: () => {
        const requestsToClean = requests.filter(
          (i) => i.kanban.column === "DONE"
        );
  ...
  return (
    <DragDropContext onDragEnd={handleDragEnd}>
      ...
      <div className={Styles.colunas}>
        <Column
          title={"NEW"}
          tasks={requests.filter((i) => i.kanban.column === "NEW")}
          id={"1"}
        />
        <Column
          title={"ON HOLDING"}
          tasks={requests.filter((i) => i.kanban.column === "ON HOLDING")}
          id={"2"}
        />
        <Column
          title={"DONE"}
          tasks={requests.filter((i) => i.kanban.column === "DONE")}
          id={"3"}
        />
