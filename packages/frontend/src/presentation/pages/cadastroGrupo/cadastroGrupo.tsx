...
const CadastroGrupo : React.FC =() => {
    ...
    const handleSolicitarFeature= () => {
      if (!solicicitarFeature){
          setSolicicitarFeature(true)
      }else{
          setSolicicitarFeature(false)
      }
    }
  
    const handleSolicitarHotfix = () => {
      if (!solicitarHotfix){
          setSolicitarHotfix(true)
      }else{
          setSolicitarHotfix(false)
      }
    }
  
    const handleAvaliarAnaliseRisco = () => {
      if (!avaliarAnaliseRisco ){
          setAvaliarAnaliseRisco (true)
      }else{
          setAvaliarAnaliseRisco (false)
      }
    }
  
    const handleAvaliarAlinhamento = () => {
      if (!avaliarAlinhamento){
          setAvaliarAlinhamento(true)
      }else{
          setAvaliarAlinhamento(false)
      }
    }
  
    const handleAnaliseObrigatoria = () => {
      if (!analiseObrigatoria){
          setAnaliseObrigatoria(true)
      }else{
          setAnaliseObrigatoria(false)
      }
    }

    const handleAlinhamentoObrigarorio = () => {
        if (!alinhamentoObrigarorio){
            setAlinhamentoObrigarorio(true)
        }else{
            setAlinhamentoObrigarorio(false)
        }
      }
      ...
      const handleRequest = async () => {
        try {
          ...
          MySwal.fire({
            html: "Grupo cadastrado com sucesso.",
            icon: "success",
            width: "350px",
            background: "#FAF0E6",
            color: "#000",
            ...
            <div className={Styles.H1cadastroGrupo}>
                ...
                <form className={Styles.formCadastroGrupo} onSubmit={handleSubmit}>
                    <div className={Styles.inputGroup}>
                        ...
                        <input className={Styles.inputText}maxLength={60} value={nomeGrupo} onChange={(e:any)=>{setNomeGrupo(e.target.value)}}></input>
                        <label htmlFor='nome'>Descrição</label>  
                        <input className={Styles.inputText}maxLength={60} value={descricaoGrupo} onChange={(e:any)=>{setDescricaoGrupo(e.target.value)}}></input>
                        <label htmlFor='nome'>Equipe</label>  
                        <div style={ { width: '100%' } }>
                        <SelectType onChange={setNomeEquipe} options={equipes} />
                        </div>
                        </div>
