        ...
        MySwal.fire({
            ...
            if (r.isConfirmed) {
                ...
                try {
                    ...
                    Swal.fire('Sucesso', `Usuário ${item.name} excluído com sucesso!`, 'success')
                    loadUsers()
                } catch(e) {
                    let errorMessage = e.response.data.message
                    if(errorMessage.includes('QueryFailedError: update or delete')){
                        errorMessage = 'Existem informações de chamados vinculadas a esse usuário, por isso não foi possível excluí-lo.'
                    }
                    Swal.fire('Erro', `Ocorreu um erro ao excluir o usuário. ${errorMessage}`, 'error')
                }
            }
          })    
    }
                    ...
                    <tbody>
                        ...
                        {data.map((item: any) => (
                            <tr key={item.user_id}>
                                ...
                                <td>
                                    <div>
                                        <i onClick={(e: any) => { handleEdit(e, item) }} className='material-icons'>
                                            create
                                        </i>
                                        <i onClick={(e: any) => { handleDelete(e, item) }}className='material-icons'>
                                            delete
                                        </i>
                                    </div>
                                </td>
